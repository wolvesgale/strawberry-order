...
export default function AdminPricesPage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage(`「${productName}」の価格を登録しました。`);
    } catch (e: any) {
      setError(e.message);
    } finally {
      setSaving(false);
    ...
    <main className="min-h-screen bg-slate-950 text-slate-50 py-10 px-4">
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">
              商品単価・税率・適用開始日を登録します。注文作成時に最新の有効価格が自動適用されます。
            </p>
          </div>
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-900/70 p-4 space-y-3">
          ...
          <form className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4" onSubmit={handleAdd}>
            ...
            <div className="sm:col-span-2 lg:col-span-4 flex items-center justify-between gap-4">
              <p className="text-[11px] text-slate-400">
                ※ 同じ商品で複数の日付を登録した場合、注文日時に最も近い過去の設定が自動適用されます。
              </p>
              <button
                type="submit"
                disabled={saving}
              ...
              <table className="min-w-full text-xs text-slate-100">
                <thead className="bg-slate-800">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right">税率</th>
                    <th className="px-3 py-2 text-right">税額（1枚）</th>
                  ...
                  {prices.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-t border-slate-800">
                        <td className="px-3 py-2 font-medium text-emerald-200">
